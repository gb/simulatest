...
/**
 * JDBC driver that routes every {@link Connection} through the Insistence
 * Layer. URLs take the form:
 *
 * <pre>
 *   jdbc:insistencelayer:&lt;underlying-jdbc-url&gt;
 * </pre>
 *
 * <p>e.g. {@code jdbc:insistencelayer:jdbc:h2:mem:app;DB_CLOSE_DELAY=-1}. The
 * underlying URL is resolved to a real driver via {@link DriverManager}, a
 * single physical connection is opened against it, and every
 * {@code connect()} call returns the Insistence Layer's {@link Connection}
 * proxy wrapping it.
 *
 * <p>Useful in environments where wrapping at the {@code DataSource} bean
 * level is impossible (e.g. frameworks that hard-bind their persistence
 * stack to a connection pool at build time and bypass standard CDI or
 * Hibernate {@code ConnectionProvider} seams). When you can wrap at the
 * {@code DataSource} layer, prefer
 * {@link InsistenceLayerDataSource} directly.
 *
 ...
 */
public final class InsistenceLayerJdbcDriver implements Driver {

	/** URL scheme prefix recognized by this driver. */
	public static final String URL_PREFIX = "jdbc:insistencelayer:";

	private static volatile String configuredUnderlyingUrl;

	static {
		try {
			DriverManager.registerDriver(new InsistenceLayerJdbcDriver());
	...
	// Synchronized so pool warm-up threads hitting connect() concurrently can't
	// race into double initialization.
	//
	...
	//
	// When the underlying InsistenceLayerJdbcDataSource fails to open (bad
	// credentials, outage), InsistenceLayerRegistry.configure throws before
	// mutating its state, so retrying with valid parameters later configures
	// cleanly.
		...
		if (InsistenceLayerFactory.isConfigured()) {
			if (!underlyingUrl.equals(configuredUnderlyingUrl)) {
				throw new SQLException(
					"InsistenceLayerJdbcDriver was already configured for '" + configuredUnderlyingUrl
					+ "'; cannot switch to '" + underlyingUrl + "' in the same JVM.");
			}
			return;
		}
		InsistenceLayerFactory.configure(new InsistenceLayerJdbcDataSource(underlyingUrl, info));
